...
Dim Mybot As RobotAPI.RobotLink
...
Call Mybot.RegisterAlert(Form1)
...
Mybot.Drive theta, SetSpeedSlider.Value
...
If Button = 1 Then
    range = Mybot.scan(mytheta, scanres)
    
    If range > 0 Then
        On Error Resume Next
        Picture1.Circle (Mybot.X, Mybot.Y), range, 0, (mytheta - scanres) / 57.3, (mytheta + scanres) / 57.3
...
If Button = 2 Then
    rsq = (dx * dx + dy * dy)
    range = Sqr(rsq)
    If Mybot.cannon(Int(mytheta), range) = -1 Then
        Text1.Text = "Bang"
    Else
        Text1.Text = "Click"
    End If
...
'    Text1.Text = Str(X) + "," + Str(Y)
    ...
    'Gauge2.Value = Mybot.heat
    Gauge3.Value = Mybot.health
    
    ActSpeedSlider.Value = Mybot.speed
