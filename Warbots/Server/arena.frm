    ...
    For i = 1 To LastBot
        ...
        If Bots(i).status <> "N" Then
            ...
            If Bots(i).newshot > 0 Then
                ' convert dx,dy to units/tick
                Bots(i).dx = Bots(i).dx / quanta
                Bots(i).dy = Bots(i).dy / quanta
                'serverbox.Text2 = "[" & Str(i) & "] " & Str(Bots(i).fire)
                ' fire is just range at this point. Shells travel 200m/sec.
                ' fire becomes number of ticks till shell hits
                ' plus 2 for explosion
                Bots(i).fire = Int(Bots(i).fire * quanta / 200) + 2
                'serverbox.Text2 = serverbox.Text2 & ", " & Str(Bots(i).fire)
                Bots(i).newshot = 0
